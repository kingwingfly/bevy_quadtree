...
cmds.spawn((
    ...
    // Spawn collision shape with `Sprite`, the plugin will auto-update it following `Changed<GlobalTransform>`
...
5. However, you need manually update collision shapes in some case
    ...
    |trigger: Trigger<TextRefreshEvent>,
     mut q_box: Query<(&mut Sprite, &mut CollisionRect)>| {
        if let Ok((mut s, mut c)) = q_box.get_mut(trigger.entity()) {
            ...
            // the plugin only track `Changed<GlobalTransform>`
